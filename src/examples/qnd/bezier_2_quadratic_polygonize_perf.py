#!/usr/bin/env python3
# test_polygonize_performance.py
# Performance test for the polygonize_quadratic_bezier methods
...
def test_python_method(steps: int):
    ...
    return BezierCurve.polygonize_quadratic_bezier_python(POINTS, steps)


def test_numpy_method(steps: int):
    """Test the NumPy implementation"""
    return BezierCurve.polygonize_quadratic_bezier_numpy(POINTS, steps)


def test_auto_method(steps: int):
    """Test the auto implementation (chooses between Python and NumPy)"""
    return BezierCurve.polygonize_quadratic_bezier(POINTS, steps)
...
def test_different_input_formats():
    ...
    tuple_points = ((0.0, 0.0), (50.0, 200.0), (200.0, 0.0))
    ...
    python_tuple = BezierCurve.polygonize_quadratic_bezier_python(tuple_points, steps)
    python_numpy = BezierCurve.polygonize_quadratic_bezier_python(numpy_points, steps)

    numpy_tuple = BezierCurve.polygonize_quadratic_bezier_numpy(tuple_points, steps)
    numpy_numpy = BezierCurve.polygonize_quadratic_bezier_numpy(numpy_points, steps)

    auto_tuple = BezierCurve.polygonize_quadratic_bezier(tuple_points, steps)
    auto_numpy = BezierCurve.polygonize_quadratic_bezier(numpy_points, steps)
...
if __name__ == "__main__":
    print("Performance Test for polygonize_quadratic_bezier methods")
    print("=" * 60)
