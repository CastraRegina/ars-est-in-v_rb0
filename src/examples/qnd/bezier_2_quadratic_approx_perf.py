#!/usr/bin/env python3
"""Benchmark script for quadratic Bezier control point approximation performance."""
...
def generate_test_data(num_points: int) -> np.ndarray:
    """Generate test points along a quadratic Bezier curve."""
    ...
    return BezierCurve.polygonize_quadratic_curve(control_points, steps=num_points - 1)
...
def benchmark_current_implementation(sizes: List[int], repeats: int = 100) -> dict:
    ...
    for size in sizes:
        # Generate test data inside loop to avoid lint warning
        test_data = generate_test_data(size)

        timer = timeit.Timer(lambda d=test_data: BezierCurve.approximate_quadratic_control_points(d))
