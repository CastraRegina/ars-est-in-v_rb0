...
def poly_func_uniform(segment):
    return polygonize_uniform(segment,
                              num_points=POLYGONIZE_UNIFORM_NUM_POINTS)


def poly_func_by_angle(segment):
    return polygonize_by_angle(segment,
...
print(f'  ...using "{POLYGONIZE_ANGLE_MAX_DEGREE}" max-angle [degree] and ' +
...
path1 = dwg.path(PATH_STRING_INPUT, stroke="black",
                 stroke_width="0.09", fill="none")
path2 = dwg.path(polygon_path_uniform, stroke="red",
                 stroke_width="0.06", fill="none")
path3 = dwg.path(polygon_path_by_angle, stroke="green",
