...
class AvPathCreator:
    ...
    def left_exterior_silhouette(
        ...
        # result_coords = AvPathCreator._remove_backside_edges(result_coords)
        result_coords = AvPathCreator._remove_upward_undercuts(result_coords)
        result_coords = AvPathCreator._remove_downward_undercuts(result_coords)
    ...
    @staticmethod
    def _remove_backside_edges(coords: np.ndarray) -> np.ndarray:
        """Remove backside edges from polygon coordinates.

        A point is removed if it creates an ascending slope between its neighbors:
        pt(n-1).y < pt(n).y < pt(n+1).y

        The method iterates over all points starting from the top-leftmost point.

        Args:
            coords: Array of polygon coordinates as (x, y) pairs

        Returns:
            np.ndarray: Filtered coordinates with backside edges removed
        """
        if len(coords) < 3:
            return coords
        ...
        # Find the top-leftmost point (start)
        max_y = max(p[1] for p in points)
        max_y_points = [(i, p) for i, p in enumerate(points) if abs(p[1] - max_y) < 1e-10]
        min_x_among_max_y = min(p[1][0] for p in max_y_points)
        start_idx = next(i for i, p in max_y_points if abs(p[0] - min_x_among_max_y) < 1e-10)

        # Create reordered list starting from start_idx
        reordered = points[start_idx:] + points[:start_idx]

        # Filter out backside edge points
        filtered = [reordered[0]]  # Always keep the first point

        # Iterate through all points
        for i in range(1, len(reordered)):
            # Get neighbors (with wrap-around)
            prev_idx = i - 1
            next_idx = (i + 1) % len(reordered)

            prev_y = reordered[prev_idx][1]
            curr_y = reordered[i][1]
            next_y = reordered[next_idx][1]

            # Check if this is a backside edge: ascending slope
            # Remove points where pt(n-1).y < pt(n).y < pt(n+1).y
            if not prev_y < curr_y < next_y:
                filtered.append(reordered[i])

        # Convert back to 3-column format (x, y, 0)
        result = np.zeros((len(filtered), 3), dtype=np.float64)
    ...
    @staticmethod
    def _remove_upward_undercuts(coords: np.ndarray) -> np.ndarray:
    ...
    @staticmethod
    def _remove_downward_undercuts(coords: np.ndarray) -> np.ndarray:
        """Remove inward undercuts from polygon coordinates.
        ...
        is reached (following CW order, i.e. the givne coords are iterated in inverse direction)
        ...
        mirrored_result = AvPathCreator._remove_upward_undercuts(mirrored_coords)
